...
def email_system():
    ...
    # Check if email service is configured (prefer app config if available)
    try:
    ...
    has_gmail = bool(gmail_user and gmail_pass)
    has_sendgrid = bool(sendgrid_key)
    has_email_service = has_gmail or has_sendgrid

    config = current_app.config

    return render_template('dashboard/email_system.html',
...
def test_email():
    ...
    # Send test email to fixed address
    recipient_email = "peterbarasa620@gmail.com"
